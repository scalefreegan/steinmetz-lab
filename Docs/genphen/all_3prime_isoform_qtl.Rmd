...
In the 3' isoform data from Chenchen (SxY) there are `r sum(do.call(c,lapply(peak_counts_05,function(i){i$npeaks})))` QTLs among `r sum(qtl_genes < 0.05)` at an FDR of 5% among `r length(peak_counts_05)` total genes. At FDR of 1% there are `r sum(do.call(c,lapply(peak_counts_01,function(i){i$npeaks})))`

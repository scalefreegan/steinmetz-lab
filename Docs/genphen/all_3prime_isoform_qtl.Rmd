...
```{r, echo=FALSE,message=FALSE,warning=FALSE,fig.height=4,fig.width=8}
pheatmap(data[c(A,B),],cluster_cols = F,cluster_rows = F,clustering_distance_rows = as.dist(cosineDist(data[c(A,B),])),scale = "row",annotation_row = data.frame(rbind(cbind(A,"S"), cbind(B,"Y")), row.names=1))
```
